...
export function runDoctor(config: FeishuConfig, connected: boolean, cardkitAvailable: boolean | string | null = null): DoctorFinding[] {
  ...
  if ((config.accessPolicy ?? "open") === "open") findings.push({ level: "warning", message: "访问策略为 open，任何可访问 Bot 的用户都可能控制当前 Pi 上下文" });
  if (config.accessPolicy === "allowlist" && !(config.allowedChatIds?.length || config.allowedOpenIds?.length)) findings.push({ level: "warning", message: "allowlist 为空，所有消息都会被拒绝" });
